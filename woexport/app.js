/* WO Volume Raw Data Table -- a Tableau dashboard extension that renders one worksheet as an interactive table:
 ...
 *
 * Same engine as the TAT Excel Export extension; everything workbook-specific is a <setting> in the twb, so the two
 * pages of this workbook share one hosted copy and differ only by their settings.
 *
 * Two differences from the TAT build:
 *   `param` defaults to EMPTY -- this workbook has no Top-N row cap yet, so the Preview box bounds the TRANSFER only
 *   and the export reads everything the dashboard filters return without touching a parameter.  Point `param` at a
 *   'Rows shown' parameter once the Top-N block exists and the old behaviour comes back with no code change.
 *   `exactColumns` -- when 'true', only the columns named in `columns` are shown, so the text-mark helper field that
 *   every Tableau text table needs does not surface as an empty trailing column.
 *
...
(function (root) {
  ...
  var DEFAULTS = {
    sheet: 'WO Table',                 // worksheet on the dashboard to read
    filename: 'Work Order Volume',     // file name prefix; a timestamp is appended
    dateFormat: 'dd/mm/yyyy',          // Excel number format for date columns
    dateTimeFormat: 'dd/mm/yyyy hh:mm',
    label: 'Excel',                    // text on the green export button
    param: '',                         // '' = no Tableau-side row cap (see the header note)
    paramAll: '10000000',
    allRows: 'true',                   // default state of the "All rows" checkbox
    pageSize: '100',                   // rows per page on screen
    previewRows: '1000',               // how many rows to transfer on open ('0' = all of them)
    sortBy: 'WO Closed Date',          // column the table opens sorted on ('' = the order Tableau returns)
    sortDir: 'desc',
    totalsSheet: '',                   // uncapped one-mark sheet with COUNTD + MIN/MAX of each date, if one exists
    exactColumns: 'true',              // show ONLY the columns named below
    // Tableau's summary data hands columns back as dimensions A-Z then measures, NOT in the worksheet's shelf order.
    // This is the order we want left to right.
  ...
  function shape(columns, rows, opts) {
    ...
    // put the columns in the requested left-to-right order; unlisted ones keep their relative place, after the
    // listed ones -- or are dropped entirely when exactColumns is on
  ...
  /** the uncapped one-mark worksheet: {CNTD(WO): n, MIN(date): …, MAX(date): …} */
  function readTotals() {
  ...
  function withAllRows(sheet, fn) {
    ...
    return dash.findParameterAsync(cfg.param).then(function (p) {
      param = p;
      if (!p) { status('Parameter "' + cfg.param + '" not found; reading every row the filters return', 'warn'); return fn(); }
  ...
  function renderPager() {
    ...
    var n = shown === loaded ? loaded : shown;
    $('count').innerHTML = all > n ? '<b>' + n.toLocaleString() + '</b> of ' + all.toLocaleString() + ' rows'
                                   : '<b>' + n.toLocaleString() + '</b> rows';
    // the wording has to match what the Preview box actually does: with a `param` it moves Tableau's own row cap,
    // without one it only bounds how many rows are transferred to this page.
    $('count').title = all > loaded
      ? (cfg.param
          ...
          : loaded.toLocaleString() + ' of ' + all.toLocaleString() + ' rows loaded for browsing.\n'
            + 'Pick a bigger Preview size to load more.\n')
  ...
  function load(first) {
    ...
    var keep = $('status').className === 'ok' ? $('status').textContent : null;
